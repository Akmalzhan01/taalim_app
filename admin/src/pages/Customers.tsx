    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-900 tracking-tight">База клиентов (CRM)</h2>
                    <p className="text-slate-500 text-sm mt-1">Постоянные клиенты и их история покупок</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Всего клиентов</p>
                ...
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Общие продажи (CRM)</p>
                        <h4 className="text-2xl font-black text-slate-900">
                            {customers.reduce((acc, c: any) => acc + c.totalPurchasedAmount, 0).toLocaleString()} <span className="text-sm">сом</span>
                        </h4>
                    </div>
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-4">
                    ...
                    <div>
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">Лучший клиент</p>
            ...
            <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden">
                <table className="min-w-full">
                    <thead className="bg-slate-50 border-b border-slate-100">
                        <tr>
                            <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Клиент</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Телефон</th>
                            <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-widest">Всего покупок</th>
                        ...
                        {sortedCustomers.map((customer: any, index: number) => (
                            <tr key={customer._id} className="hover:bg-slate-50/80 transition-colors group">
                                <td className="px-6 py-5 whitespace-nowrap">
                                    <div className="flex items-center">
                                        ...
                                        <div className="ml-4">
                                            ...
                                            <div className="text-[10px] text-slate-400 font-medium">Добавлен: {new Date(customer.createdAt).toLocaleDateString()}</div>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5 whitespace-nowrap">
                                ...
                                <td className="px-6 py-5 whitespace-nowrap text-sm font-black text-indigo-600">
                                    {customer.totalPurchasedAmount.toLocaleString()} <span className="text-[10px] text-slate-400">сом</span>
                                </td>
                                <td className="px-6 py-5 whitespace-nowrap">
                                ...
                                <td className="px-6 py-5 whitespace-nowrap">
                                    <span className="text-xs font-bold text-slate-500 bg-slate-50 px-2 py-1 rounded-lg border border-slate-100">
                                        {customer.branch?.name || 'Центральный'}
                                    </span>
                                </td>
                                <td className="px-6 py-5 whitespace-nowrap text-right text-sm font-medium">
                                    <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button onClick={() => handleEdit(customer)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-xl transition-all" title="Редактировать">
                                            <Edit size={18} />
                                        </button>
                                        <button
                ...
                {sortedCustomers.length === 0 && !isLoading && (
                    <div className="p-20 text-center">
                        ...
                        <h3 className="text-lg font-bold text-slate-900">Клиенты не найдены</h3>
                        <p className="text-slate-500 text-sm mt-1">В результате поиска или в выбранном филиале нет клиентов</p>
                    </div>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="bg-white rounded-[40px] w-full max-w-md overflow-hidden relative z-10 shadow-2xl animate-in zoom-in-95 duration-200">
                        <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                            <div>
                                <h3 className="text-2xl font-black text-slate-900 tracking-tight">{isEditMode ? 'Редактировать клиента' : 'Новый клиент'}</h3>
                        ...
                        <form onSubmit={handleSubmit} className="p-8 space-y-6">
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-2 block">Имя клиента</label>
